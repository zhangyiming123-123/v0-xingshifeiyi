...
const pages: Record<string, {
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          {
            label: '沿习成俗，代代相传',
            body: '此后，人们便将这次"���兽"的经历口口相传，并约定成俗：每逢年节喜庆，必舞狮以驱邪纳吉，保一方平安。这个充满生活智慧的民间传说，至今仍是解释岭南醒狮民俗由来最广为人知的故事。',
          },
        ],
      },
      {
  ...
  lionhead: {
    title: '狮头制作',
    subtitle: 'Crafting the Lion Head',
    heroImage: '/images/lion-history-2.jpg',
    heroAlt: '岭南醒狮彩绘华丽的狮头近景，展示精湛扎作工艺',
    accentColor: '#C9A96E',
    sections: [
      {
        heading: '工序一·扎——骨架成型，千点定神',
        icon: '扎',
        items: [
          {
            label: '精选竹材',
            body: '首选广东清远或广宁生长3至5年的老桂竹，冬至后采伐，水分少、质地坚韧且不易生虫，再剖成上百根粗细、长短各不相同的篾条，作为狮头骨架的原料。',
          },
          {
            label: '开篾与软化',
            body: '将竹篾剖削至不同规格，关键部位（如主骨架）的篾条需用烛火烤软，以增强韧性使其易于弯曲造型，为后续精准成形奠定基础。',
          },
          {
            label: '扎成形架',
            body: '以鼻梁、额头、腮骨主架为起点，用浸湿纱纸条将交叉竹篾扎结固定并刷上浆糊，形成约1300个扎点。精益求精的匠人恪守"额宽额高3:2比例、狮耳对称45度"的严格规范，骨架的精准与否决定了狮头最终的气韵。',
          },
        ],
      },
      {
        heading: '工序二·扑——披纱挂绸，强基固形',
        icon: '扑',
        items: [
          {
            label: '多层裱糊',
            body: '在骨架上均匀涂抹特制浆糊，层层裱上纱纸和纱布。传统狮头扑多达六层纱纸，现代工艺常用"两层纱纸＋一层纱布＋一层纱纸"的组合，兼顾强度与轻便。',
          },
          {
            label: '精塑五官',
            body: '从狮头背部、前额等大面积区域开始，再细致处理鼻、角、眼窝等复杂部位。每粘贴一层都要仔细抚平，糊好的狮头置阴凉处自然风干，表皮总厚度通常控制在2毫米以内，轻盈而坚固。',
          },
        ],
      },
      {
        heading: '工序三·写——彩笔传神，赋予性格',
        icon: '写',
        items: [
          {
            label: '脸谱化身',
            body: '岭南狮头汲取了传统粤剧脸谱精髓：黄底刘备狮象征仁义尊贵，红底关羽狮代表忠义胜利，黑底张飞狮寓意勇猛霸气。选择什么颜色的狮头，就是选择一种"性格"和"气场"。',
          },
          {
            label: '刚劲纹样',
            body: '匠人用毛笔绘制唐草纹（象征繁荣）、虎斑纹（象征辟邪）、火焰纹（装饰感极强）等纹样，线条刚劲有力，色彩对比强烈，层次分明，每一笔都是匠人数十年功力的体现。',
          },
        ],
      },
      {
        heading: '工序四·装——点睛配饰，神形兼备',
        icon: '装',
        items: [
          {
            label: '点睛与动感装配',
            body: '贴上能灵活转动的活动眼珠并装上眼帘使狮子能够"眨眼"传神；装上能活动的下巴，部分高端狮头还装有让耳朵晃动的"机关"与发声铜铃，使整个狮头随舞动者动作活灵活现。',
          },
          {
            label: '华美装饰',
            body: '额头中央镶嵌代表智慧的铜镜，狮角、狮耳等部位装上色彩各异的绒毛球。最终整个狮头固定在藤圈之上，作为舞狮者的操控把手，一件完整的艺术品就此诞生。',
          },
        ],
      },
      {
        heading: '颜色与性格：六大狮头色系',
        icon: '色',
        items: [
          {
            label: '黄狮（刘备狮）——仁义帝王',
            body: '五彩唐草花纹，象征仁义、尊贵、祥和、睿智。适用于大型庆典、迎宾、喜庆节日等正式场合，代表最高礼遇。',
          },
          {
            label: '红狮（关羽狮）——忠义胜利',
            body: '黑眉黑须，额上"二龙争珠"或"金钱"纹，象征忠义、勇敢、财富、胜利。最常见的狮头颜色，适用于开业庆典、祝寿、竞赛，寓意红红火火、旗开得胜。',
          },
          {
            label: '黑狮（张飞狮）——勇猛霸气',
            body: '黑脸黑须，青鼻红额，额绘"火焰"或"八卦"纹，象征勇猛、好斗、刚烈、辟邪。适用于打擂、斗狮、驱邪除煞，民间有"黑狮最狠"之说。',
          },
          {
            label: '绿狮（赵子龙狮）·蓝狮（马超狮）·白狮（黄忠狮）',
            body: '绿狮忠勇机智，寓意生机勃勃；蓝狮刚毅沉稳，相对少见；白狮沉稳智谋，代表黄忠老当益壮，纯白极为罕见。现代竞技中亦出现粉、紫、靛蓝等非传统色，但文化核心仍以红、黄、黑、绿、蓝五大正色为基础。',
          },
        ],
      },
    ],
    closing: '一颗狮头，凝聚了匠人数日乃至数周的心血。从精选一根竹篾，到最后点上那一对会眨眼的眼珠，每一道工序都是对这门千年技艺的虔诚守护。',
  },
